 ...
 * 
 * 1.0.1a 
 *		 Custom changes added for QNA sakai tool
 *
...
jQuery.iNestedSortable = {
	...
	newCheckHover : function (e) {
		...
		/* Begin: QNA custom code */

		var dragged = jQuery.iDrag.dragged;
		
		// Stops adding category to other category
		if (precItem) {
			if ((dragged.id.substring(0,8) == 'category' && !dragged.id.match('question')) && precItem.id.match('question')) {
				quit = true;
			}
			
			// No questions in root
			if (!shouldNest && dragged.id.match('question') && (precItem.id.substring(0,8) == 'category' && !precItem.id.match('question'))) {
				quit = true;
			}
		}  else {
			// Question can't be first
			if (touchingFirst && dragged.id.match('question')) {
				quit = true;
			}
		}
	...
	shouldNestItem : function(e, precedingItem ) {
		...
		/* Begin: QNA custom code */
		// Categories should not nest
		var dragged = jQuery.iDrag.dragged;
		
		if (dragged.id.substring(0,8) == 'category' && !dragged.id.match('question')) {
			return false;
		}
		/* End: QNA custom code */
		...
		/* Begin: QNA custom code */	
		var nLevelPrec = jQuery(precedingItem).parents("." + e.sortCfg.accept.split(" ").join(".")).length;
		// Checks that "There can be only one!"..... level of nesting
		if (nLevelPrec >= 1) return false;
		/* End: QNA custom code */
